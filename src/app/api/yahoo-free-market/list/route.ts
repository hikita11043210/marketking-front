...
export async function GET(request: Request) {
    try {
        ...
        const response = await serverFetch(`/api/v1/yahoo-free-market/list?${queryParams.toString()}`, {
            cache: 'no-store',
        });
...
export async function DELETE(request: Request) {
    try {
        ...
        const response = await serverFetch(`/api/v1/yahoo-free-market/delete?sku=${sku}`, {
            method: 'DELETE',
            cache: 'no-store',
        });
        ...
        if (!response.ok || !data.success) {
            return NextResponse.json({
                error: 'yahoo_free_market_delete_failed',
